...
export class UpdateList extends TranslatingComponent {
    ...
    findRelatedField(item){
        let otherFields = [item];
        item.relatedFields && item.relatedFields.forEach(relatedField=>{
            let otherField = _.find(this.props.updates, i=>i.fieldToCompare ===relatedField);
            if(otherField){
                otherFields.push(otherField);
            }
        });
        return otherFields;
    }
    ...
    handleUpdateClick(item){
        this.props.onupdate && this.props.onupdate(this.findRelatedField(item));
    }
    ...
    handleIgnoreClick(item){
        this.props.onignore && this.props.onignore(this.findRelatedField(item));
    }
    handleUndoIgnoreClick(item){
        this.props.onundoignore && this.props.onundoignore(this.findRelatedField(item));
    }
    ...
    handleUndoUpdateClick(item){
        this.props.onundoupdate && this.props.onundoupdate(this.findRelatedField(item));
    }
    ...
    render(){
        ...
        if(!this.props.updates){
            ...
            if(this.props.updates.length == 0){
                ...
                list = this.props.updates.filter(item=>!item.notVisible).map(item => {
                    ...
                    return <tr key = {item.fieldToCompare}>

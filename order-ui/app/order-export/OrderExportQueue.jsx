...
import { OrderExportQueueFilter } from './OrderExportQueueFilter';
import { OrderExportQueueList } from './OrderExportQueueList';

export class OrderExportQueue extends TranslatingComponent {
    constructor(props) {
        super(props);
        this.state = {
    ...
    onSearch(filter) {
        ...
        OrderQueueService.orderExportSearch(filter).then(response => {
    ...
    onHandleRequeue(row) {
        OrderQueueService.orderExportRequeue(row.id).then(response => {
    ...
    render() {
        return (
            <div>
                <PageHeader title="Export Order Queue"   translate={true}  />
                <OrderExportQueueFilter filter={this.state.filter} onSearch={(filter) => this.onSearch(filter)} />
                <LoaderWrapper busy={this.state.isSearchBusy}>
                    <OrderExportQueueList data={this.state.searchResult}

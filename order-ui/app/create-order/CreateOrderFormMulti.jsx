...
import { ManufacturerValidator, PartyValidator } from "./steps/validation/OrderParticipantValidator";
...
export class CreateOrderFormMulti extends TranslatingComponent {
    constructor(props) {
        super(props);
        this.state = {
            rate: 0,
    ...
    getRequestedDeliveryDate(shipmentIndex) {
        ...
        if (order.customer && order.truckLoadType && order.serviceType && readyDate && shipment.sender && shipment.consignee) {
            ...
            let request = {
                ...
                loadingCompanyType: _.defaultTo(_.get(shipment.sender,'handlingCompany.type'), 'COMPANY'),
                unloadingCompanyType: _.defaultTo(_.get(shipment.consignee,'handlingCompany.type'), 'COMPANY')
            };
            ProjectService.calculateDeliveryDate(request).then(response => {
                let deliveryDateValue = response.data.deliveryDate && response.data.deliveryDate + " " + shipment.consignee.handlingLocationTimezone;
                this.handleChange(`shipments[${shipmentIndex}].deliveryDateTime`, deliveryDateValue);
    ...
    createOrderSteps(props) {
        let steps = [];
        let { order, template } = props;
        if (order && order.shipments && order.shipments[0].senderCustomizations.length !== 1) {
            steps.push({
                title: "Please select a sender",
                id: this.getStepIdForShipment("sender", 0),
                component: () => <Steps.Party options={order.shipments[0].senderCustomizations}
                    customer={template.customer}
                    messageForNoParty="Service Type or Load Type is not appropriate with Sender Customization"
                    showSearchInput={true}
                    searchFields={['company.name', 'companyLocation.name', 'handlingCompany.name', 'handlingLocation.name']}
                    size={6} />,
                onChange: (key, value) => this.handleSenderChange(key, 0, value),
                onInactive: () => this.getRequestedDeliveryDate(0),
                validate: (value) => {
                    return PartyValidator.validate(value, order.truckLoadType, order.serviceType, order.shipments[0].consignee)
                }
            });
        }

        if(order && order.shipments && order.shipments[0].sender && order.shipments[0].sender.manufacturerOption && order.shipments[0].sender.manufacturerOption.code !== 'DONT_ASK'){
            if('ASK' === order.shipments[0].sender.manufacturerOption.code){
                steps.push({
                    title: "Do you have Manufacturer Company Info?",
                    id: this.getStepIdForShipment("haveManufacturer", 0),
                    component: () => <Steps.YesNoButtonStep />,
                    onChange: (key,value)=> this.handleManufacturerOptionChange(key, 0, value, order.shipments[0].sender.manufacturers)
                })
            } 
            if((order.shipments[0].haveManufacturer || 'REQUIRED' === order.shipments[0].sender.manufacturerOption.code) && 1 !== order.shipments[0].sender.manufacturers.length){
                steps.push({
                    title: "Please select a manufacturer",
                    id: this.getStepIdForShipment("manufacturer", 0),
                    component: () => <Steps.Manufacturer options={order.shipments[0].sender.manufacturers}
                        showSearchInput={true} searchFields={['company.name', 'companyLocation.name']} size={6} />,
                    validate: (value) => ManufacturerValidator.validate(value)
                });
            }
        }

        if (order && order.shipments && order.shipments[0].consigneeCustomizations.length !== 1) {
            steps.push({
                title: "Please select a consignee",
                id: this.getStepIdForShipment("consignee", 0),
                component: () => <Steps.Party options={order.shipments[0].consigneeCustomizations}
                    customer={template.customer}
                    messageForNoParty="Service Type or Load Type is not appropriate with Consignee Customization"
                    showSearchInput={true}
                    searchFields={['company.name', 'companyLocation.name', 'handlingCompany.name', 'handlingLocation.name']}
                    size={6} />,
                ...
                onInactive: () => this.getRequestedDeliveryDate(0),
                validate: (value) => {
                    return PartyValidator.validate(value, order.truckLoadType, order.serviceType, order.shipments[0].sender)
                }
            });
        }

        if (props.order.customerIsPartner && props.order.originalCustomers && props.order.originalCustomers.length !== 1) {
            steps.push({
    ...
    createShipmentSteps(props, shipmentIndex) {
        ...
        if (shipmentIndex > 0 && template) {
            ...
            if(shipment.sender && shipment.sender.manufacturerOption && shipment.sender.manufacturerOption.code !== 'DONT_ASK'){
                ...
                if((shipment.haveManufacturer || 'REQUIRED' === shipment.sender.manufacturerOption.code) && shipment.sender.manufacturers.length != 1){
                    steps.push({
                        title: "Please select a manufacturer",
            ...
            if (shipment.consigneeCustomizations && shipment.consigneeCustomizations.length !== 1) {
                steps.push({
                    ...
                    component: () => <Steps.Party options={shipment.consigneeCustomizations}
                        customer={template.customer}
                        messageForNoParty="Service Type or Load Type is not appropriate with Consignee Customization"
                        showSearchInput={true}
                        searchFields={['company.name', 'companyLocation.name', 'handlingCompany.name', 'handlingLocation.name']}
        ...
        if (shipmentIndex === 0) {
            if (props.order.truckLoadTypes && props.order.truckLoadTypes.length > 1) {
                steps.push({
            ...
            if (props.order.serviceTypes && props.order.serviceTypes.length > 1) {
                steps.push({
        ...
        if(shipment.askLoadingOrderNumbers) {
            steps.push({
                title: "Do you have loading order numbers?",
                id: this.getStepIdForShipment("haveLoadingOrderNumbers", shipmentIndex),
                component: () => <Steps.YesNoButtonStep />,
                onChange: (key, value) => this.handleOrderNumbersValue(key, shipmentIndex, value, "loadingOrderNumbers")
            });
            if (shipment.haveLoadingOrderNumbers) {
                steps.push({
                    title: "Please enter loading order numbers",
                    id: this.getStepIdForShipment("loadingOrderNumbers", shipmentIndex),
                    component: () => <Steps.OrderNumbersStep />,
                    validate: Validator.EmptyValidator.validate
                });
            }
        ...
        if (shipment.askConsigneeOrderNumbers) {
            ...
            if (shipment.haveConsigneeOrderNumbers) {
                steps.push({
                    ...
                    component: () => <Steps.OrderNumbersStep hasNumbers={shipment.haveConsigneeOrderNumbers} />,
                    validate: Validator.EmptyValidator.validate
                });
            }
        }

        if(shipment.askUnloadingOrderNumbers) {
            steps.push({
                title: "Do you have unloading order numbers?",
                id: this.getStepIdForShipment("haveUnloadingOrderNumbers", shipmentIndex),
                component: () => <Steps.YesNoButtonStep />,
                onChange: (key, value) => this.handleOrderNumbersValue(key, shipmentIndex, value, "unloadingOrderNumbers")
            });
            if (shipment.haveUnloadingOrderNumbers) {
                steps.push({
                    title: "Please enter unloading order numbers",
                    id: this.getStepIdForShipment("unloadingOrderNumbers", shipmentIndex),
                    component: () => <Steps.OrderNumbersStep />,
                    validate: Validator.EmptyValidator.validate
                });
            }
        ...
        if (shipment.haveUnloadingAppointment) {
            steps.push({
                ...
                component: () => <UnloadingAppointment deliveryDateTime={shipment.deliveryDateTime}
                    readyDate={shipment.loadingAppointment ? shipment.loadingAppointment.startDateTime : (shipment.readyDateTime && shipment.readyDateTime.value)}
                ...
                validate: value =>
                    Validator.AppointmentDateValidator.validate(value,
                        shipment.loadingAppointment ?
                            ...
                            ) : (shipment.readyDateTime && shipment.readyDateTime.value), shipment.deliveryDateTime)
            });
        }
...
CreateOrderFormMulti.contextTypes = {

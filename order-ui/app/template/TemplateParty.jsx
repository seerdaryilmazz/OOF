...
import { Kartoteks, ProjectService } from '../services';
import { ManufacturerCompany } from './order/ManufacturerCompany';
...
export class TemplateParty extends TranslatingComponent {
    ...
    componentDidMount(){
        if(this.props.party && this.props.party.company && this.props.party.company.id){
            this.loadCompanyData(this.props.party.company.id);
        }
        if(this.props.party && this.props.party.handlingCompany && this.props.party.handlingCompany.id){
            this.loadHandlingCompanyData(this.props.party.handlingCompany.id);
        }
    }
    componentWillReceiveProps(nextProps){
        let currentCompanyId = _.get(this.props, "party.company.id");
        let nextCompanyId = _.get(nextProps, "party.company.id");
        if(nextCompanyId && currentCompanyId !== nextCompanyId){
            this.loadCompanyData(nextCompanyId);
        }
        let currentHandlingCompanyId = _.get(this.props, "party.handlingCompany.id");
        let nextHandlingCompanyId = _.get(nextProps, "party.handlingCompany.id");
        if(nextHandlingCompanyId && currentHandlingCompanyId !== nextHandlingCompanyId){
            this.loadHandlingCompanyData(nextHandlingCompanyId);
        }
        this.loadLookups(nextProps.type);
    }
    ...
    loadHandlingCompanyData(companyId){
        axios.all([
            Kartoteks.getCompanyLocations(companyId),
            Kartoteks.getCompanyContacts(companyId)
    ...
    render(){
        ...
        return(
            <Grid>
                ...
                <GridCell width = "1-3">
                    <Checkbox label="Ask Order Numbers" value = {party.askOrderNumbers}

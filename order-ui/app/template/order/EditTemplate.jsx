...
import { ConsigneePartyRules, SenderPartyRules } from './PartyRules';

export class EditTemplate extends TranslatingComponent {

    constructor(props) {
        super(props);
        this.state = { templateFields: [], rules: {} };
        this.loadLookupData();
        this.loadCustomerRules(this.props.template);
    }
    ...
    componentWillReceiveProps(nextProps) {
        if(!_.isEqual(nextProps.template, this.props.template)){
            this.loadCustomerRules(nextProps.template);
    ...
    loadCustomerRules(template) {
        axios
            .all([ProjectService.getDeliveryDateRulesForCustomer(template.customer.id)])
            .then(axios.spread((rdd) => {
                this.setState({
                    rules: {
                        rdd: rdd.data
                    }
    ...
    loadCompanyLocations(companyId) {
        Kartoteks.getCompanyLocations(companyId).then(response => {
            this.setState({
                ownerLocations: response.data.map(item => convertLocationsWithPostalCodes(item))
            });
        }).catch(error => {
            Notify.showError(error);
        });
    ...
    handleAddDefaultField(field) {
        ...
        let values = []
        let customizations = this.props.template.senderCustomizations.concat(this.props.template.consigneeCustomizations);
        customizations.forEach(customization => {
            values = _.union(values, _.get(customization.customizedDefaults, field));
        });
    ...
    handleFieldItemUnselected(item, field, addBack) {
        if (this.checkIsCustomized(field, item)) {
            Notify.showError("The option cannot be deleted.It is used by a customized template");
            addBack && addBack(true);
            return true;
        }
        return false;
    }

    checkIsCustomized(field, item) {
        let customizedValues = [];
        let customizations = this.props.template.senderCustomizations.concat(this.props.template.consigneeCustomizations);
        customizations.forEach(customization => {
            customizedValues = _.union(customizedValues, _.get(customization.customizedDefaults, field));
        });
        return 0 <= _.findIndex(customizedValues, { code: item.code });
    }

    openModal(){
        this.setState({ isModalOpen:true}, ()=> this.modal.open() );
    }
    closeModal(){
        this.setState({ isModalOpen:false}, ()=> this.modal.close() );
    }
    ...
    render() {
        ...
        return (
            <div>
                <PageHeader title={`Customer: ${customerName}`} />
                <Card>
                    <Grid>
                        <GridCell width="1-1">
                        ...
                        <GridCell width="1-3">
                            {   this.props.template && this.props.template.customer
                                    ?<Button label="RDD Rule" style={_.isEmpty(this.state.rules.rdd) ? "muted" : "primary"} onclick={()=>this.openModal()} />
                                    :null}
                        </GridCell>
                        <GridCell width="1-3" />
                        <GridCell width="1-3">
                        ...
                        <GridCell width="2-3" />
                        <GridCell width="1-4">
                            <Checkbox label="Ask Customer Order Numbers" value={template.askOrderNumbers}
                        ...
                        <GridCell width="1-1">
                            <EditTemplateCustomizedCompany customizations={template.senderCustomizations}
                                rules = {SenderPartyRules}
                                partyType="sender"
                                defaults={template.defaults} lookup={this.state.lookup}
                                fields={this.state.templateFields}
                                onChange={(value) => this.handleChange("senderCustomizations", value)} />
                        </GridCell>
                        <GridCell width="1-1">
                            <EditTemplateCustomizedCompany customizations={template.consigneeCustomizations}
                                rules= {ConsigneePartyRules}
                                partyType="consignee"
                                defaults={template.defaults} lookup={this.state.lookup}
                                fields={this.state.templateFields}
                                onChange={(value) => this.handleChange("consigneeCustomizations", value)} />
                        </GridCell>
                        <GridCell width="1-1">
                ...
                <Modal ref={c=>this.modal = c}
                    id={this.props.template.customer.id}
                    ...
                    large={true}>
                        {this.state.isModalOpen 
                            ? <ResponsiveFrame    src={`/ui/order/modal-delivery-date-rules?customerId=${this.props.template.customer.id}`}

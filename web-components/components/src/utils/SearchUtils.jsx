...
export class SearchUtils {
    ...
    matcher(option, input) {
        ...
        if (this.searchFields) {
            _.each(this.searchFields, field => {
                ...
                if(value){
                    if (_.isString(value) && 0 <= this.translate(value).toLocaleUpperCase(navigator.language).search(this.make_pattern(input))) {
                        keys.push(field);
    ...
    search(searchValue, list) {
        let value = _.defaultTo(searchValue,'').split(" ").filter(o=>!_.isEmpty(o));
        let filtered = list;
        value.forEach(token=>{
            filtered = _.intersection(filtered, _.filter(list, o => this.matcher(o, token)));
        })
        return _.isEmpty(value)? list : filtered;
    }
}
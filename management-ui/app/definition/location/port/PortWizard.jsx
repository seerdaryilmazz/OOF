...
export class PortWizard extends TranslatingComponent {
    ...
    initialize(){
        axios.all([
            PortService.listRegistrationMethods(),
            PortService.listAssetTypes()
    ...
    initializeState(props) {
        if (props.location.query && props.location.query.id) {
            this.loadPort(props.location.query.id);
        }else{
    ...
    loadPort(id){
        PortService.get(id).then(response => {
            let state = _.cloneDeep(this.state);
    ...
    handleCancelClick(){
        this.context.router.push('/ui/management/port-list');
    }
    ...
    updateAssets(assets) {
        let place = _.cloneDeep(this.state.place);
        place.assets = assets;
        this.setState({place: place}, () => this.savePort());
    }

    savePort(){
        PortService.save(this.state.place).then(response => {
            Notify.showSuccess("Port saved");
            this.context.router.push('/ui/management/port-list');
        }).catch(error => {
            Notify.showError(error);
        })
    ...
    render() {
        ...
        let title = super.translate("New Port");

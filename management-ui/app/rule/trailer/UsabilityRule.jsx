...
export class UsabilityRule extends TranslatingComponent {
    ...
    handleSaveRule(){
        ...
        if(this.state.rule.id){
            ...
            if(_.find(rules, {documentType: {code: this.state.rule.documentType.code}, tripType: {code: this.state.rule.tripType.code}})){
                Notify.showError("There is already a rule for " + this.state.rule.documentType.name + " and " + this.state.rule.tripType.name);
                return;
            }else{
        ...
        TrailerRuleService.saveUsabilityRule(rules).then(response => {
                    ...
                    <li key = {item.id}>
                        <span className="md-list-heading">
                            <a href="#" onClick = {(e) => this.handleSelectRule(e, item)}>
                                {this.context.translator.mf.compile(text)([item.documentType.name, item.daysDue, item.tripType.name])}
                            </a>
                        </span>
                ...
                <table className="uk-table">
                    ...
                    <tr>
                        ...
                        <td className="uk-vertical-align-bottom" style = {style} width="20%">
                            <DropDown options = {this.props.lookups.documentTypes} value = {this.state.rule.documentType}
                                      onchange = {(value) => this.updateState("documentType", value)}
                                      required = {true}/>
                        </td>
                        <td className="uk-vertical-align-bottom" style = {style}>

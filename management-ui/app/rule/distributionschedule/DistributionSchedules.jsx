...
import {DistributionScheduleRuleService, CommonService, ZoneService} from "../../services";
...
export class DistributionSchedules extends TranslatingComponent {
    ...
    componentDidMount(){
        axios.all([
            DistributionScheduleRuleService.getDistributionSchedules(),
    ...
    loadList(preSelectedItemId){
        DistributionScheduleRuleService.getDistributionSchedules().then(response => {
    ...
    handleSave(){
        ...
        DistributionScheduleRuleService.saveDistributionSchedule(value).then(response => {
            Notify.showSuccess("Distribution schedule saved");
            this.loadList(value.id);
    ...
    handleDelete(){
        let value = this.state.selectedItem;
        if (value) {
            UIkit.modal.confirm("Are you sure?", () => {
                DistributionScheduleRuleService.deleteDistributionSchedule(value).then(response => {
